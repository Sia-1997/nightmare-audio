...
file_name = './audio/joram-moments_of_clarity-08-solipsism-59-88.wav'
...
def calc_loss(thisInput, model):
  ...
  this_class = layer_activations[0,1]
...
batch1Out1 = (np.power(2.0, batch1)-1.0)/10000.0
print('b')
print(batch1Out1)
'''

'''T: Helper for getting layer output tensor'''
'''
audio_out3 = librosa.feature.inverse.mel_to_audio(M=batch1Out1.T,
                                               sr=config.SR,
                                               hop_length=config.FFT_HOP,
                                               n_fft=config.FFT_SIZE)

librosa.output.write_wav('outaudio/audio_out1.wav', audio_out1, config.SR)
librosa.output.write_wav('outaudio/audio_out2.wav', audio_out2, config.SR)
librosa.output.write_wav('outaudio/audio_out3.wav', audio_out3, config.SR)
